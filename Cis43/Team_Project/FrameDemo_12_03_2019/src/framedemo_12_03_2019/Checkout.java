...
public class Checkout extends JFrame {
    ...
    public ArrayList<String> book_list = new ArrayList<String>();
    ...
    public int k = 0; // identifier for Search popup
    ...
    public Checkout(ArrayList<String> borrow_list) { 
        ...
        Border operBorder = BorderFactory.createTitledBorder("Available Books");
        panel2.setBorder(operBorder);

        
        book_list.add("Calculus 3A");
        book_list.add("Calculus 3B");
        book_list.add("Physics 4A");
        book_list.add("Physics 4B");
        book_list.add("Chemistry 1A");
        book_list.add("Chemistry 1B");

        for (int i = 0; i < 6; i++)
        {
            if (!(borrowed_book_list.contains(book_list.get(i)))) 
            {
                JCheckBox book = new JCheckBox(book_list.get(i));
                books.add(book);
            }
            else book_list.remove(i);
        }
    ...
    private class Buttonhandler1 implements ActionListener {
        ...
        if(event.getSource() == button3)
        {
           ...
           JOptionPane.showMessageDialog(null, "The book is returned to the library");
           dispose();
        ...
        else if (event.getSource() == button2)
        {
            ...
            {
                String input_text = textField1.getText();
                String input = input_text.substring(0, 1).toUpperCase() + input_text.substring(1).toLowerCase();
                System.out.println(input);
                for (int i = 0; i < books.size(); i++) {
                    System.out.println(get_borrowed_book_list());
                    if (book_list.get(i).contains(input))
                    {
                ...
                if (k == 1) {
                    ...
                    JOptionPane.showMessageDialog(null, "The books are available");
                    k = 0;
                }
            }
                ...
                for (int i = 0; i < getBooks().size(); i++) {
                    if (getBooks().get(i).isSelected()) {
                        ...
                        if (selectedBooks.length() % 6 == 0) {
                            String space = "<br><blockquote><blockquote>";
                            selectedBooks += space;
                        }
