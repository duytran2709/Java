...
public class BinarySearchTree < T extends Comparable < ? super T >>
{
    ...
    private boolean isBalanced(BinaryNode<T> rootNode)
    {
    	...
    	int leftHeight = 0;
    	int rightHeight = 0;
    	if (rootNode.hasLeftChild()) leftHeight = rootNode.getLeftChild().getHeight();
    	if (rootNode.hasRightChild()) rightHeight = rootNode.getRightChild().getHeight();
    	
    	boolean checkHeight = (leftHeight - rightHeight <= 1) && (rightHeight - leftHeight <= 1);
    	return checkHeight && isBalanced(rootNode.getLeftChild()) &&
    			isBalanced (rootNode.getRightChild());
    }
    ...
    private boolean isBST(BinaryNode<T> rootNode, T min, T max)
    {
    	...
    	if (max != null && rootData.compareTo(max) > 0) return false;
    	if (min != null && rootData.compareTo(min) < 0) return false;
    	
    	boolean leftCheck = true;
    	if (rootNode.hasLeftChild())
    	{
    		leftCheck = isBST (rootNode.getLeftChild(), min, rootData);
    	}
    	
    	boolean rightCheck = true;
    	if (rootNode.hasRightChild())
    	{
    		rightCheck = isBST (rootNode.getRightChild(), rootData, max);
    	}
    ...
    public T getPredecessor(T entry)
    {
    	...
    	T smallestEntry = rootNode.getData();
    	rootNode = root;
    	if (entry.equals(smallestEntry))
    	{
    		return entry;
    	}
    	else
    	{
    		return getPredecessor (rootNode, null, entry);
    	}
    	
    }
    private T getPredecessor (BinaryNode<T> rootNode, T pred, T entry)
    {
    	if (rootNode == null) return pred;
    	
    	if (rootNode.getData().equals(entry))
    	{
    		if (rootNode.getLeftChild() != null)
    		{
    			return findLargest (rootNode.getLeftChild()).getData();
    		}
    	}
    	else if (rootNode.getData().compareTo(entry) > 0)
    	{
    		return getPredecessor (rootNode.getLeftChild(), pred, entry);
    	}
    	else
    	{
    		pred = rootNode.getData();
    		return getPredecessor (rootNode.getRightChild(), pred, entry);
    	}
    	return pred;
    }
